...
public class StartTrashCommand implements CommandExecutor {

    private final TrashPlugin plugin;

    public StartTrashCommand(TrashPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (this.plugin.getTrashManager().isActive()) {
            sender.sendMessage(CC.colour("&cThe trash dropper is already active!"));
            return false;
        }

        this.plugin.getTrashManager().startTrash();
        sender.sendMessage(CC.colour("&aYou have successfully started the trash dropper!"));
        return true;
    }
}
